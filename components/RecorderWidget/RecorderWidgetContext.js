...
export const RecorderWidgetProvider = ({ children }) => {
  ...
  useEffect(() => {
    const handleStateChange = () => {
      ...
      if (
          (currentRouteName === RECORD_STATE.NEW_RECORD || currentRouteName === RECORD_STATE.PREVIEW_RECORD) &&
          !isOpen
      ) {
  ...
  const showRecorderWidget = (replace = false) => {
    ...
    navigation.navigate(RECORD_STATE.NEW_RECORD, { replace });
    setIsOpen(true);
  };

  const showPreviewRecorderWidget = () => {
    setState(RECORD_STATE.PREVIEW_RECORD);
    navigation.navigate(RECORD_STATE.PREVIEW_RECORD);
  };

  const hideRecorderWidget = () => {
    setState(RECORD_STATE.INIT);
    navigation.navigate(RECORD_STATE.INIT);
    setIsOpen(false);
  };
  ...
  const goBack = () => {
    if (state === RECORD_STATE.PREVIEW_RECORD) {
      setState(RECORD_STATE.NEW_RECORD);
      navigation.navigate(RECORD_STATE.NEW_RECORD);
